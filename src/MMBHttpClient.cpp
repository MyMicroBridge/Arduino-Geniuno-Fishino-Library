...
const char* MMBHttpClient::kUserAgent = "Arduino/2.2.0";
const char* MMBHttpClient::kContentLengthPrefix = HTTP_HEADER_CONTENT_LENGTH ": ";

#ifdef PROXY_ENABLED // currently disabled as introduces dependency on Dns.h in Ethernet
MMBHttpClient::MMBHttpClient(Client& aClient, const char* aProxy, uint16_t aProxyPort)
...
#else
MMBHttpClient::MMBHttpClient(Client& aClient)
...
void MMBHttpClient::stop()
{
  iClient->stop();
  resetState();
}

void MMBHttpClient::beginRequest()
{
  iState = eRequestStarted;
}

int MMBHttpClient::startRequest(const char* aServerName, uint16_t aServerPort, const char* aURLPath, const char* aHttpMethod, const char* aUserAgent)
{
...
int MMBHttpClient::startRequest(const IPAddress& aServerAddress, const char* aServerName, uint16_t aServerPort, const char* aURLPath, const char* aHttpMethod, const char* aUserAgent)
{
    tHttpState initialState = iState;
    if ((eIdle != iState) && (eRequestStarted != iState))
    {
...
int MMBHttpClient::sendInitialHeaders(const char* aServerName, IPAddress aServerIP, uint16_t aPort, const char* aURLPath, const char* aHttpMethod, const char* aUserAgent)
{
#ifdef LOGGING
...
void MMBHttpClient::sendHeader(const char* aHeader)
{
    iClient->println(aHeader);
}

void MMBHttpClient::sendHeader(const char* aHeaderName, const char* aHeaderValue)
{
...
void MMBHttpClient::sendHeader(const char* aHeaderName, const int aHeaderValue)
{
    iClient->print(aHeaderName);
    iClient->print(": ");
    iClient->println(aHeaderValue);
...
void MMBHttpClient::sendBasicAuth(const char* aUser, const char* aPassword)
{
...
void MMBHttpClient::finishHeaders()
{
    iClient->println();
    iState = eRequestSent;
}

void MMBHttpClient::endRequest()
{
...
int MMBHttpClient::responseStatusCode()
{
    if (iState < eRequestSent)
    {
        return HTTP_ERROR_API;
...
int MMBHttpClient::skipResponseHeaders()
{
...
bool MMBHttpClient::endOfBodyReached()
{
...
int MMBHttpClient::read(uint8_t *buf, size_t size)
{
